...
**Prototype:** Working. **Hiện trạng (18/9):** web `codebase/web/` → API `codebase/api/` → AI thật `openai/gpt-4o-mini` (`codebase/ai/`); trace ở `eval/traces/`. Chạy end-to-end trên trình duyệt không can thiệp tay. **Phần MOCK (ghi rõ trên giao diện):** đối thủ ở chế độ "Ghép trận 1v1" (mô phỏng ở frontend), rating và bảng xếp hạng (lưu trên trình duyệt, người chơi khác là dữ liệu mẫu), nút "Demo offline" (câu hỏi viết sẵn, không gọi AI). **Bản thử cho demo, ngoài bộ đo và quality bar:** nút "📖 Hiểu sâu hơn về câu này" sau khi trả lời (chế độ luyện) — AI tổng hợp từ khoá + điểm cần phân biệt + "vì sao lựa chọn của bạn chưa đúng" từ slide, kèm khái niệm liền kề; giao diện ghi "bản thử, chưa qua bước kiểm tra như câu hỏi". **Chưa khai trên giao diện, sẽ sửa:** hồ sơ đấu của chính người chơi đang khởi tạo sẵn số liệu mẫu (rating 1240, 13 trận, 8 thắng) thay vì 0 trận.
...
|---|---|---|
| Một vòng | Rule | Kiểu Kahoot: hiện **đề trước, ẩn đáp án**, đếm 4-3-2-1 (4 s) → hiện đáp án + **10 s trả lời**, chạm là nộp → **cả hai đã chọn** thì lộ kết quả đối thủ → hiện kết quả vòng, **đếm ngược 4 s rồi tự sang câu** (không có nút). Hết giờ chưa chọn → 0 điểm, xem đáp án đúng + giải thích |
...
Giới hạn đã biết: đề + 4 lựa chọn thật dài trung vị 36 / 84 / 100 chữ ở mức 1 / 2 / 3 (run-2 + run-4), đọc hết mất 11–14 / 25–34 / 30–40 s. 4 s đọc đề + 10 s trả lời (≈ 14 s) vẫn **chặt với câu mức 2–3** → nhiều câu có thể hết giờ; cần đo tỷ lệ hết giờ khi người dùng thật chơi và chỉnh `BATTLE_READ_SECONDS` / `BATTLE_ANSWER_SECONDS` trong `app.js`.
...
| G9 - cho sửa lỗi | "Báo câu sai" (ghi log, thay câu, không tính điểm); luyện thường có "Đổi câu khác" tối đa 2 lần mỗi lượt; chế độ đấu không cho đổi câu |
...
| Correction | POST /report hoặc /skip | Báo sai: loại câu, ghi log, thay câu (không giới hạn, mọi lần báo đều ghi log); ở luyện thường, đổi câu giữ khái niệm và mức, **tối đa 2 lần mỗi lượt**, hết thì khoá nút và nhắc "cứ chọn đáp án bạn thấy đúng nhất"; chế độ đấu ẩn nút đổi câu |
...
| 19/09 00:34 | **Bản thử "Hiểu sâu hơn" (demo, ngoài bộ đo):** `POST /explain` (chỉ câu đã trả lời, mỗi câu gọi AI 1 lần) + khung mở theo nhu cầu dưới phần nguồn: từ khoá, điểm cần phân biệt, vì sao chọn sai, khái niệm liền kề + slide + "Luyện 3 câu". Không có ở chế độ đấu | Ý tưởng của Nam cho demo, trả lời góp ý dùng thử "câu LLM sinh ra có chất lượng hơn quiz VLearn không". Test API 68/68 (+5). Thử trên Edge: nút chỉ hiện sau khi trả lời, mở không làm nhảy trang, mở lại không gọi AI lần 2, chế độ đấu không có. AI thật: bám slide ở 2 câu thử, chờ 3,4–5,5 s. **Chưa đo trong bộ đo** |
| 18/09 23:41 | **Chế độ đấu: thời gian hiện đề trước khi hiện đáp án 3 → 4 s** (đếm 4-3-2-1); 10 s trả lời giữ nguyên | Dùng thử (`validation/log.md`): 2/2 người chơi chế độ đấu thấy "thời gian đọc nhanh quá"; Nam chọn tăng thời gian hiện đề lên 4 s |
| 18/09 22:04 | **Sửa tiêu đề kết quả trận:** chỉ ghi "sát nút" khi cách biệt < 500 điểm (điểm tối thiểu của 1 câu đúng); còn lại ghi "Bạn chiến thắng!" / "Đối thủ thắng trận này"; nhãn hiện số điểm cách biệt ("Kém 1.550 điểm") | Nam phát hiện: thua cách xa vẫn ghi "Đối thủ thắng sát nút". Thử 6 trường hợp điểm: đúng cả thắng/thua xa, thắng/thua sát, hòa |
